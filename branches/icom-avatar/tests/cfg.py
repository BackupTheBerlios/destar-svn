...
"""
An unittest Suite for destar_cgf.py upgrading using module unittest
"""
...
class TestCfg(unittest.TestCase):
    ...
    def setUp(self):
        self.files = fnmatch.filter(os.listdir(CONFIGLETS_DIR),"cfg_*.py")
	print self.files
    ...
    def testAllStringsDynamic(self):
        ...
        pbxRegex = re.compile(r'VarType\("pbx"')
        cfgRegex = re.compile(r'^class Cfg(.*)\(')
        ...
        for fi in self.files:
            ...
            for line in file(fi):
                if state == 0:
                    s = pbxRegex.search(line)
                    s2 = cfgRegex.search(line)
                    if s:
                        if s.group(1) != "":
                            partialSuccess = 1
                        state = 1
