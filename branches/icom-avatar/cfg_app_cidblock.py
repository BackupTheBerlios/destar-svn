...
class CfgAppCIDBlocking(CfgApp):
	...
	description = _("Extensions to add/remove from personal CID blocking list. The add extension will be of the form prefix+2_digits_key+*+number_to_block. The remove extension will be of the form prefix+2_digits_key+*+number_to_block.")
	...
	def createAsteriskConfig(self):
		...
		c.appendExten("_%sXX*X." % self.set, "Set(DB(CIDBLOCKLIST/${CALLERID(num)}/${EXTEN:%d:2})=${EXTEN:%d})" % (len(self.set),len(self.set)+3))
		c.appendExten("_%sXX*X." % self.set, "Hangup")
		c.appendExten("_%sXX*X" % self.unset, "DBdel(CIDBLOCKLIST/${CALLERID(num)}/${EXTEN:%d:2})" % len(self.unset))
		c.appendExten("_%sXX*X" % self.unset, "Hangup")
