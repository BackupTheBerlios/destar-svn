...
class CfgPhoneSip(CfgPhone):
	...
	def createVariables(self):
		self.variables = [
			...
			VarType("tags",
					title=_("Etiquetas"),
					hint=_("Separadas por comas y sin espacios"),
					optional=True,
					len=256),
			...
			VarType("codec",
					title=_("Force CODEC (g729, gsm, etc)"),
					optional=True,
					len=4),
			...
			VarType("pin",
					...
					type="int",
					optional=True,
					default=int(random.random()*10000),
					len=6),
	...
	def createAsteriskConfig(self):
		...
		sip.append("subscribecontext=%s" % pbx)
                sip.append("call-limit=20")

		if self.codec:
			sip.append('disallow=all')
			sip.append('allow=%s' % self.codec)
