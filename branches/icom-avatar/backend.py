...
def initializeAsteriskConfig():
	...
	* macros.inc
	...
	c = AstConf("smb.conf")
	c.setSection("global")
	c.append("workgroup = AVATAR")
	c.append("server string = Servidor de Telefonia ICOM")
	...
	c.setSection("fax")
	c.append("path = /var/spool/asterisk/fax/")
	c.append("valid users = faxes")
	c.append("force user = root")
        c.append("read only = no")


	c.setSection("monitoreo-monitor")
	c.append("path = /var/spool/asterisk/monitor/")
	c.append("valid users = monitoreo")
	c.append("force user = root")
        c.append("read only = no")

	c.setSection("admin-fax")
	c.append("path = /var/spool/asterisk/fax/")
	c.append("valid users = admin")
	c.append("force user = root")
        c.append("read only = no")
