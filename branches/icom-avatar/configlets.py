...
class AsteriskConfigFile:
	...
	def __init__(self, fn):
		...
		if fn == 'faxreceive.conf':
			fn = os.path.join(CONF_DIR, 'asterisk_faxreceive.conf', fn)
		if fn == 'system.conf':
			fn = os.path.join("/etc/dahdi", fn)
...
class ConfigletTree:
	...
	def getPhoneByExtension(self, ext):
		for obj in self:
			try:
				if obj.ext == ext:
					return obj
...
class Cfg(Holder):
	...
	def checkConfig(self):
		...
		May be overridden for additional test."""
                ...
                try:
                        if self.name == "0":
                                return ("name", _("Name can't be cero"))
                except AttributeError:
                        pass
...
class CfgPhone(Cfg):
	...
	def head(self):
		return (_("Type"), _("Extension"), _("Name"), _("Caller-Id Name"), _("Virtual PBX"))
	...
	def row(self):
		try:
			...
			pbx = _('None')
		try:
			callerid = self.calleridname
		except AttributeError:
			callerid = _('None')
		return (self.shortName, ext, self.name, callerid, pbx)
	...
	def createExtensionConfig(self):
		...
		for tmp_ext in (self.ext, self.name):
			...
			extensions.appendExten(tmp_ext, "Set(CDR(pbx)=%s)" % (pbx), context=pbx)
			extensions.appendExten(tmp_ext, "Set(CDR(userfield)=%s)" % (self.name), context=pbx)
			extensions.appendExten(tmp_ext, "Set(__PICKUPMARK=%s)" % (self.ext), context=pbx)
			if tags:
				extensions.appendExten(tmp_ext, "Set(SPYGROUP=%s)" % (tags.split(',')[0]), context=pbx)
	...
	def createOutgoingContext(self):
		...
		c.append("exten=_X!,n,Set(fw_ext=${DB(CFBS/%s/${CALLERID(num)})})" % pbx)
		c.append('exten=_X!,n,GotoIf($["${fw_ext}" != ""]?cfbs)')
		c.append("exten=_X!,n,Set(fw_ext=${DB(CFTO/%s/${CALLERID(num)})})" % pbx)
		c.append('exten=_X!,n,GotoIf($["${fw_ext}" != ""]?cfto)')
		c.append('exten=_X!,n,Goto(real-out-%s,${EXTEN},1)' % self.name)
		c.append('exten=_X!,n(cfbs),Macro(call-forward-warning,${DB(CFBS/%s/${CALLERID(num)})},call-fwd-on-busy)' % pbx)
		c.append('exten=_X!,n,Goto(real-out-%s,${EXTEN},1)' % self.name)
		c.append('exten=_X!,n(cfim),Macro(call-forward-warning,${DB(CFIM/%s/${CALLERID(num)})},call-fwd-unconditional)' % pbx)
		c.append('exten=_X!,n,Goto(real-out-%s,${EXTEN},1)' % self.name)
		c.append('exten=_X!,n(cfto),Macro(call-forward-warning,${DB(CFTO/%s/${CALLERID(num)})},call-fwd-no-ans)' % pbx)
		c.append('exten=_X!,n,Goto(real-out-%s,${EXTEN},1)' % self.name)
		c.append("exten=_X.,1,Set(fw_ext=${DB(CFIM/%s/${CALLERID(num)})})" % pbx)
		c.append('exten=_X.,n,GotoIf($["${fw_ext}" != ""]?cfim)')
		c.append("exten=_X.,n,Set(fw_ext=${DB(CFBS/%s/${CALLERID(num)})})" % pbx)
		...
		c.append('exten=_X.,n(cfto),Macro(call-forward-warning,${DB(CFTO/%s/${CALLERID(num)})},call-fwd-no-ans)' % pbx)
		c.append('exten=_X.,n,Goto(real-out-%s,${EXTEN},1)' % self.name)
		c.append("exten=_*.,1,Set(fw_ext=${DB(CFIM/%s/${CALLERID(num)})})" % pbx)
		c.append('exten=_*.,n,GotoIf($["${fw_ext}" != ""]?cfim)')
		c.append("exten=_*.,n,Set(fw_ext=${DB(CFBS/%s/${CALLERID(num)})})" % pbx)
		c.append('exten=_*.,n,GotoIf($["${fw_ext}" != ""]?cfbs)')
		c.append("exten=_*.,n,Set(fw_ext=${DB(CFTO/%s/${CALLERID(num)})})" % pbx)
		c.append('exten=_*.,n,GotoIf($["${fw_ext}" != ""]?cfto)')
		c.append('exten=_*.,n,Goto(real-out-%s,${EXTEN},1)' % self.name)
		c.append('exten=_*.,n(cfbs),Macro(call-forward-warning,${DB(CFBS/%s/${CALLERID(num)})},call-fwd-on-busy)' % pbx)
		c.append('exten=_*.,n,Goto(real-out-%s,${EXTEN},1)' % self.name)
		c.append('exten=_*.,n(cfim),Macro(call-forward-warning,${DB(CFIM/%s/${CALLERID(num)})},call-fwd-unconditional)' % pbx)
		c.append('exten=_*.,n,Goto(real-out-%s,${EXTEN},1)' % self.name)
		c.append('exten=_*.,n(cfto),Macro(call-forward-warning,${DB(CFTO/%s/${CALLERID(num)})},call-fwd-no-ans)' % pbx)
		c.append('exten=_*.,n,Goto(real-out-%s,${EXTEN},1)' % self.name)


		contextout = "real-out-%s" % self.name 
		c.setSection(contextout)
		...
		for obj in configlet_tree:
			if obj.__class__.__name__ == 'CfgDialoutNormal':
				try:
					if self.__getitem__("dialout_"+obj.name):
						...
						try:
							...
							tags = self.name
						c.appendExten("%s" % obj.pattern,"Set(CDR(userfield)=%s)" % (tags), context=contextout)
						c.appendExten("%s" % obj.pattern,"Set(SPYGROUP=%s)" % (tags.split(',')[0]), context=contextout)
				...
				c.appendExten("_%sXX" % prefix,"Set(dest=${DB(QUICKDIALLIST/${CALLERID(num)}/${EXTEN:%d})})" % len(prefix), e="Playback(invalid)", context=contextout)
