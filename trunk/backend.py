...
def initializeAsteriskConfig():
	...
	* macros.inc
	...
	c.setSection("master")
	c.append("table => cdr")
	c.append("columns => clid,src,dst,dcontext,channel,dstchannel,lastapp,lastdata,start,answer,end,duration,billsec,disposition,amaflags,accountcode,uniqueid,userfield,pbx")
	c.append("values => '${CDR(clid)}','${CDR(src)}','${CDR(dst)}','${CDR(dcontext)}','${CDR(channel)}','${CDR(dstchannel)}','${CDR(lastapp)}','${CDR(lastdata)}','${CDR(start)}','${CDR(answer)}','${CDR(end)}','${CDR(duration)}','${CDR(billsec)}','${CDR(disposition)}','${CDR(amaflags)}','${CDR(accountcode)}','${CDR(uniqueid)}','${CDR(userfield)}','${CDR(pbx)}'")
	...
	c.append("exten=s,105,Macro(voicemail,u${ARG4}@${ARG5})")
	...
	c.append("exten=s,206,Set(CDR(pbx)=${ARG5})")
	c.append("exten=s,207,Dial(${ARG1},${dsec},Tt${dopt})")
	...
	c.append("exten=s,306,SetVar(dopt=r)")
	c.append("exten=s,307,Goto(206)")
	...
	c.append("exten=s,313,Macro(voicemail,${vmopt}${ARG4}@${ARG5})")
