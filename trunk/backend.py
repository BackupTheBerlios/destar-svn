...
def initializeAsteriskConfig():
	...
	* macros.inc
	...
	c.append('exten=s,2,GotoIf($["${fw_ext}" != ""]?fw)')
	...
	c.append('exten=s,6,GotoIf($["${dsec}" = ""]?nodsec)')

	c.append("; got dial seconds")
	c.append("exten=s,7,Set(dopt=${DIAL_OPTIONS})")
	c.append('exten=s,8,GotoIf($["${dopt}" = ""]?nodopt)')

	c.append("; got dial options")
	c.append("exten=s,9,Dial(${ARG1},${dsec},Tt${dopt})")

	c.append("; Dial result was 'timeout'")
	c.append("exten=s,11,Set(fw_ext=${DB(CFBS/${ARG5}/${ARG4})})")
	c.append('exten=s,12,GotoIf($["${fw_ext}" != ""]?fw)')
	...
	c.append("exten=s,n(nodsec),Set(dsec=45)")
	c.append("exten=s,n,Goto(7)")
	c.append("exten=s,n(nodopt),Set(dopt=r)")

	c.append("exten=s,n,Goto(9)")
	c.append("exten=s,n(vm),Macro(voicemail,$vmopt${ARG4}@${ARG5})")
